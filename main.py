...
def get_retriever_for_doc_type(file_path: str, doc_type: str):
    ...
    if doc_type == "Financial Report":
        ...
        bm25_retriever.k = 3
        return EnsembleRetriever(retrievers=[bm25_retriever, similarity_retriever], weights=[0.5, 0.5])

    elif doc_type == "Academic Paper":
        splitter = SemanticChunker(embeddings, breakpoint_threshold_type="percentile")
        split_docs = splitter.split_documents(docs)
        vector_store = Chroma.from_documents(split_docs, embeddings)
        return vector_store.as_retriever(search_type="mmr", search_kwargs={"k": 3, "fetch_k": 20})
